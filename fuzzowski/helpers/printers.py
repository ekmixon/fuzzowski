...
def repr_input_bytes_as_python(input_bytes, orig, first=False, last=False):
    try:
        ...
        if input_bytes != orig \
            ...
            if mod == 0:
                representation += f"{repr(orig)} * {mult} +"
            else:
                representation += f"{repr(orig)} * {mult} + {repr(input_bytes[-mod:])}"
        elif input_bytes != orig \
                and len(input_bytes) > 0 and input_bytes == (chr(input_bytes[0]) * len(input_bytes)).encode('utf8'):
            representation += (
                f"{chr(input_bytes[0]).encode('utf8')} * {len(input_bytes)} +"
            )
...
def block_to_python(block: Mutant, indent=0, mutant=None, first=False) -> str:
    ...
    if hasattr(block, 'stack'):
        block_code += f'{space * 2}#{block.__class__.__name__} - {block.name}\n'
        for child_block in block.stack:
            block_code += block_to_python(child_block, indent=indent*2, mutant=mutant)

            if child_block == mutant:
                block_code += f' (mutant, size={len(mutant.render())}, orig val: {child_block.original_value})'
...
def request_to_python(block: Request, indent=0) -> str:
    block_code = f"{block.name.replace('-', '_').replace('.', '_').replace(' ', '_')} = (\n"
...
def print_packets(path: list, nodes: dict) -> None:
    ...
    for e in path[:-1]:
        node = nodes[e.dst]
        p = node.render()
        line = f"{node.name.replace('-', '_')} = {repr(p)}"
...
def get_connect_python(socket_conn: SocketConnection) -> str:
    ...
    if socket_conn.proto in ["tcp", "ssl"]:
        ...
        if socket_conn.bind:
            code += f'sock.bind(({repr(socket_conn.bind[0])},{socket_conn.bind[1]}))\n'
    ...
    if socket_conn.proto in ["tcp", "ssl"]:
        ...
        code += f'\nsock.connect(({repr(socket_conn.host)}, {socket_conn.port}))\n'
...
def get_send_python(socket_conn: SocketConnection, varname: str) -> str:
    ...
    code = ''
    if socket_conn.proto in ["tcp", "ssl"]:
        code += f"sock.send({varname.replace('-', '_')})\n"
    elif socket_conn.proto == "udp":
        code += f"sock.sendto({varname.replace('-', '_')}, ({repr(socket_conn.host)}, {socket_conn.port}))\n"

    elif socket_conn.proto == "raw-l2":
        code += f"sock.sendto({varname.replace('-', '_')}, ({repr(socket_conn.host)}, 0))\n"

    elif socket_conn.proto == "raw-l3":
        code += f"sock.sendto({varname.replace('-', '_')}, ({repr(socket_conn.host)}, {repr(socket_conn.ethernet_proto)}, 0, 0, {repr(socket_conn.l2_dst)}))\n"
